...
public class BufferPool {
    ...
    // 总的可用内存是nonPooledAvailableMemory和空闲字节缓冲区的数目* poolableSize的总和
    ...
    public ByteBuffer allocate(int size, long maxTimeToBlockMs) throws InterruptedException {
        ...
        try {
            ...
            // 如果申请的内存大小等于poolableSize，并且free队列不为空，直接从free队列中取出一个ByteBuffer
            ...
            if (this.nonPooledAvailableMemory + freeListSize >= size) {
                ...
                // 有足够的内存，直接分配内存
                // 将ByteBuffer队列中的ByteBuffer释放掉加入到nonPooledAvailableMemory
                freeUp(size);
                // 申请的内存大小减去size
                ...
                try {
                    ...
                    // 将condition加入到waiters队列中
                    this.waiters.addLast(moreMemory);
                    // loop over and over until we have a buffer or have reserved
                    // enough memory to allocate one
                    // 一直循环直到我们有一个buffer或者保留足够的内存来分配一个
                    while (accumulated < size) {
                        ...
                        if (accumulated == 0 && size == this.poolableSize && !this.free.isEmpty()) {
                            ...
                            // 需要分配内存，但是我们可能只会在这次迭代中得到部分我们需要的内存
                            // 释放内存队列的内存，加入到空闲内存中
                    ...
                    // 当此循环无法成功终止时，不要丢失可用内存
                    // 抛异常出来的时候，加上分配的内存，移除等待队列
