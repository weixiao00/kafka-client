...
public class RecordAccumulator {
    ...
    // 用于存储空闲的 bufferPool，包含两部分：相等大小的buffer，一个没有分的内存
    private final BufferPool free;
    private final Time time;
    private final ApiVersions apiVersions;
    // 存储每个 TopicPartition 对应的 ProducerBatch 队列
    // TopicPartition重写了hashCode和equals方法，所以可以作为key
    private final ConcurrentMap<TopicPartition, Deque<ProducerBatch>> batches;
    // 添加完成的batch
    private final IncompleteBatches incomplete;
    // The following variables are only accessed by the sender thread, so we don't need to protect them.
    // 用来存储当前的 TopicPartition正在处理
    ...
    public RecordAppendResult append(TopicPartition tp,
        ...
        try {
            ...
            // 获取或创建 TopicPartition 对应的 ProducerBatch 队列
            Deque<ProducerBatch> dq = getOrCreateDeque(tp);
            synchronized (dq) {
                if (closed)
                    throw new KafkaException("Producer closed while send in progress");
                // 尝试将消息体添加到 ProducerBatch 中
            ...
            synchronized (dq) {
                ...
                // 创建一个新的 MemoryRecordsBuilder
                MemoryRecordsBuilder recordsBuilder = recordsBuilder(buffer, maxUsableMagic);
                // 创建一个新的 ProducerBatch, 添加MemoryRecordsBuilder
                ProducerBatch batch = new ProducerBatch(tp, recordsBuilder, nowMs);
                // 将消息体添加到 ProducerBatch 中，没有成功报异常
                FutureRecordMetadata future = Objects.requireNonNull(batch.tryAppend(timestamp, key, value, headers,
                        callback, nowMs));

                // 将 ProducerBatch 添加到队列尾部
                dq.addLast(batch);
    ...
    private RecordAppendResult tryAppend(long timestamp, byte[] key, byte[] value, Header[] headers,
        ...
        // 获取队列的最后一个 ProducerBatch
        ProducerBatch last = deque.peekLast();
        if (last != null) {
            // 尝试添加到 ProducerBatch 中
    ...
    public ReadyCheckResult ready(Cluster cluster, long nowMs) {
        ...
        for (Map.Entry<TopicPartition, Deque<ProducerBatch>> entry : this.batches.entrySet()) {
            // 获取每个 TopicPartition 对应的 ProducerBatch 队列
            Deque<ProducerBatch> deque = entry.getValue();
            ...
            if (leader == null) {
                ...
                // 不知道leader分区在哪个broker上，但是有消息可以发送
                unknownLeaderTopics.add(part.topic());
            } else if (!readyNodes.contains(leader) && !isMuted(part)) {
                // 不包含leader节点，且分区没有被处理
    ...
    private List<ProducerBatch> drainBatchesForOneNode(Cluster cluster, Node node, int maxSize, long now) {
        ...
        do {
            ...
            // 获取 TopicPartition 对应的 ProducerBatch 队列
            ...
            synchronized (deque) {
                ...
                // 要发送的总大小超过了 maxSize(默认1M)。直接返回需要发的batch消息
                // 意思是要1M的消息，但是一个batch的消息已经超过1M了，直接返回这个batch的1M消息
    ...
    private Deque<ProducerBatch> getOrCreateDeque(TopicPartition tp) {
        // 从队列里获取 TopicPartition 对应的 ProducerBatch 队列
        Deque<ProducerBatch> d = this.batches.get(tp);
        if (d != null)
            return d;
        // 如果队列不存在，则创建一个新的队列，并将其放入 batches 中
        d = new ArrayDeque<>();
    ...
    public void deallocate(ProducerBatch batch) {
        // 从 incomplete 中移除 ProducerBatch
