...
public class Sender implements Runnable {
    ...
    private long sendProducerData(long now) {
        ...
        // 如果有任何领导者尚未知晓的分区，则强制元数据更新
        if (!result.unknownLeaderTopics.isEmpty()) {
            // The set of topics with unknown leader contains topics with leader election pending as well as
            // topics which may have expired. Add the topic again to metadata to ensure it is included
            // and request metadata update, since there are messages to send to the topic.
            //未知领导者的主题集包含等待领导者选举的主题以及
            //可能已经过期的主题。再次将主题添加到元数据中，以确保包含该主题
            //请求元数据更新，因为有消息要发送到主题。
        ...
        //如果我们有任何节点准备好发送+有可发送的数据，用0超时轮询，这样就可以立即发送
        //循环并尝试发送更多数据。否则，超时时间将是下一个批处理过期之间的较小的值
        //时间，检查数据可用性的延迟时间。注意，节点可能拥有尚未存在的数据
        //可发送的，由于逗留，后退等这特别不包括具有可发送数据的节点
        //还没有准备好发送，因为它们会导致繁忙的循环。
        ...
        if (!result.readyNodes.isEmpty()) {
            ...
            //如果一些分区已经准备好发送，选择时间将为0;
            //如果某个分区已经积累了一些数据，但还没有准备好，
            //选择的时间将是现在和它的超时时间之间的时间差;
            //否则，选择时间将是现在和元数据过期时间之间的时间差;
            pollTimeout = 0;
        }
        // 每个node节点需要发送的所有分区batch消息
    ...
    private void sendProduceRequest(long now, int destination, short acks, int timeout, List<ProducerBatch> batches) {
        ...
        for (ProducerBatch batch : batches) {
            ...
            // 将所有要发送的batch消息添加到ProduceRequestData中
