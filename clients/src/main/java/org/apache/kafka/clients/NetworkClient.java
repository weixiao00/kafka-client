...
public class NetworkClient implements KafkaClient {
    ...
    public List<ClientResponse> poll(long timeout, long now) {
        ...
        // 处理接收到的数据。更新了集群的元数据(每个topic的分区数据)
     ...
     * 选择未完成请求最少的节点，至少符合连接条件。这个方法将
     * 首选具有现有连接的节点，但可能会选择尚未拥有连接的节点
     * 如果所有现有连接都在使用，则* connection。如果不存在连接，此方法将优先选择一个节点
     * 最近连接尝试最少。此方法永远不会选择不存在的节点
     * 已存在的连接，并且我们在重新连接回退期间断开了连接，或处于活动状态
     * 正在节流的连接。
     * 返回运行中请求最少的节点
     */
    @Override
